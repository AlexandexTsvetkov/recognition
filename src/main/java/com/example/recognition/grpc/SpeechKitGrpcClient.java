package com.example.recognition.grpc;

import com.example.recognition.conf.RecognitionConfig;
import com.example.recognition.model.AudioFormat;
...
public class SpeechKitGrpcClient {
    ...
    public String recognizeAudio(byte[] audioData, AudioFormat format, RecognitionType recognitionType) {
        try {
            ...
            yandex.cloud.api.operation.OperationOuterClass.Operation operation = asyncRecognizerStub.recognizeFile(request);
            ...
            throw new com.example.recognition.exception.SpeechRecognitionException("Recognition request failed", e);
        }
    }
    ...
    public String getRecognitionResult(String operationId) {
        try {
            ...
            log.debug("Starting to fetch recognition results for operation: {}", operationId);
            ...
            if (!result.isEmpty()) {
                log.debug("Final recognized text: {}", result);
                return result;
            } else {
            ...
            throw new com.example.recognition.exception.SpeechRecognitionException(
                    "Failed to get recognition result", e);
        }
    }
    ...
    private void processFinalResponse(Stt.AlternativeUpdate finalUpdate, StringBuilder recognizedText) {
        if (!finalUpdate.getAlternativesList().isEmpty()) {
            ...
            if (!finalText.isEmpty()) {
                ...
                log.debug("Final text added: {}", finalText);
            }
        }
    }
    ...
    private Stt.RecognizeFileRequest buildRecognitionRequest(byte[] audioData, AudioFormat format,
        ...
        // Настройки текстовой нормализации с учетом типа распознавания
        ...
        // Ограничение по языку для улучшения точности
